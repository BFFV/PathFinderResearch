...
public:
    ...
    // Returns nullptr if the text search index was not found
    TextSearchIndex* get_text_search_index(const std::string& name);
    ...
    const boost::unordered_flat_map<std::string, TextSearchIndexMetadata>& get_name2metadata() const
    {
    ...
    const boost::unordered_flat_map<std::string, std::unique_ptr<TextSearch::TextSearchIndex>>&
        get_name2text_search_index() const
    {
        return name2text_search_index;
    }
...
private:
    ...
    boost::unordered_flat_map<std::string, std::unique_ptr<TextSearchIndex>> name2text_search_index;
    // Name to text search index metadata (normalize, tokenize, predicate, etc...)
    boost::unordered_flat_map<std::string, TextSearchIndexMetadata> name2metadata;
