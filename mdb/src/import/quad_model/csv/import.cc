...
)
{
    ...
    {
        char* string_buffer = reinterpret_cast<char*>(MDB_ALIGNED_ALLOC(StringManager::MAX_STRING_SIZE));
...
{
    while (external_strings_end + str.size() + StringManager::MIN_PAGE_REMAINING_BYTES
               + StringManager::MAX_LEN_BYTES + 1
           >= external_strings_capacity)
    {
        // Duplicate buffer
        char* new_external_strings = reinterpret_cast<char*>(MDB_ALIGNED_ALLOC(external_strings_capacity * 2)
        );
    ...
    // copy string
    std::memcpy(&external_strings[external_strings_end] + bytes_for_len, str.data(), str.size());
