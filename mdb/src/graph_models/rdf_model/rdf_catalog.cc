    ...
    Catalog(filename)
{
    ...
    if (auto model_id = read_uint64(); model_id != MODEL_ID) {
        throw WrongModelException("RdfCatalog", MODEL_ID, model_id);
    }
    if (auto version_id = read_uint64(); version_id != VERSION) {
        throw WrongCatalogVersionException("RdfCatalog", VERSION, version_id);
    }
...
{
    if (has_changes || text_search_index_manager.has_changes() || hnsw_index_manager.has_changes()) {
        save();
    }
}
