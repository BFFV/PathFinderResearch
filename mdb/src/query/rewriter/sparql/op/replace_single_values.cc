...
void ReplaceSingleValues::visit(OpBasicGraphPattern& op_bgp)
{
    ...
    // Nothing to assign in text searches right now
    /*
    for (auto& text_search : op_bgp.text_searches) {
        text_search.accept_visitor(*this);
    }
    */

    for (auto& hnsw_seearch : op_bgp.hnsw_searches) {
        hnsw_seearch.accept_visitor(*this);
    }
}
...
void ReplaceSingleValues::visit(OpHNSWSearchIndex& op_hnsw_serach)
{
    if (op_hnsw_serach.query_id.is_var()) {
        try_replace(op_hnsw_serach.query_id);
    }
}
