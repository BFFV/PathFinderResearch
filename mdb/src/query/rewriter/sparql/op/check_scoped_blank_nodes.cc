...
void CheckScopedBlankNodes::visit(OpBasicGraphPattern& op_basic_graph_pattern) {
    ...
    for (const auto& text_search : op_basic_graph_pattern.text_searches) {
        for (const auto& var : text_search.get_all_vars()) {
            if (get_query_ctx().is_blank(var)) {
                scope_blank_nodes.insert(var);
            }
        }
    }
    for (const auto& hnsw_search : op_basic_graph_pattern.hnsw_searches) {
        for (const auto& var : hnsw_search.get_all_vars()) {
            if (get_query_ctx().is_blank(var)) {
                scope_blank_nodes.insert(var);
            }
        }
