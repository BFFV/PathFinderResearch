 ...
 */
parser grammar SparqlUpdateParser;

options{
tokenVocab=SparqlUpdateLexer;
language=Cpp;
}

query
    : prologue update (';'? prologue update)* EOF
    ;
...
createTextIndex:
    CREATE TEXT INDEX string ON iri (
        WITH (
...
createHnswIndex : // numerics must be greater than 0
    CREATE HNSW INDEX string ON iri WITH DIMENSION INTEGER MAX_EDGES INTEGER CANDIDATES INTEGER METRIC string;
...
triplesSameSubject
    ...
    : graphTerm propertyListNotEmpty | triplesNode propertyList
    ;
...
// triplesSameSubjectPath
    ...
    // : graphTerm propertyListPathNotEmpty | triplesNodePath propertyListPath
    // ;
...
propertyListPathNotEmpty
    ...
    : verb objectListPath (';' propertyListPathNotEmptyList?)*
    ;
