...
Any QueryVisitor::visitShowQuery(SparqlParser::ShowQueryContext* ctx) {
    ...
    } else if (ctx->HNSW() != nullptr) {
        type = OpShow::Type::HNSW_SEARCH_INDEX;
    } else {
...
Any QueryVisitor::visitTriplesBlock(SparqlParser::TriplesBlockContext* ctx) {
    ...
    current_op = std::make_unique<OpBasicGraphPattern>(
        ...
        std::move(current_hnsw_searches)
    );
    return 0;
}
            ...
            if (is_mdb_procedure) {
                ...
                if (curr_iri_str_suffix == MDBExtensions::Procedure::TEXT_SEARCH_SUFFIX_IRI) {
                    for (const auto& objectPath : ctx->objectListPath()->objectPath()) {
                        ...
                        current_text_searches.emplace_back(current_subject, args);
                        handled_as_procedure = true;
                    }
                } else if (curr_iri_str_suffix == MDBExtensions::Procedure::HNSW_SEARCH_SUFFIX_IRI) {
                    for (const auto& objectPath : ctx->objectListPath()->objectPath()) {
                        const auto args = parse_context_as_procedure_args<ObjectPathContextTraits>(objectPath);
                        current_hnsw_searches.emplace_back(current_subject, args);
                        handled_as_procedure = true;
                    }
                } else {
