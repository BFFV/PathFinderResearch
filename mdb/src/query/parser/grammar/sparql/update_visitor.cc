...
#include "graph_models/common/datatypes/tensor_operations.h"
...
Any UpdateVisitor::visitCreateHnswIndex(SUP::CreateHnswIndexContext* ctx) {
    auto hnsw_index_name = stringCtxToString(ctx->string(0));

    // Check if tensor index existed before
    // TODO: Enable
    // if (rdf_model.catalog.hnsw_index_manager.get_hnsw_index(hnsw_index_name) != nullptr) {
    //     throw QueryException("Tensor index \"" + hnsw_index_name + "\" already exists");
    // }

    // Check if tensor index is being created twice
    for (const auto& op : op_update->updates) {
        if (auto op_create_text_index = dynamic_cast<OpCreateHnswIndex*>(op.get())) {
            if (op_create_text_index->hnsw_index_name == hnsw_index_name) {
                throw QueryException(
                    "Tensor index \"" + hnsw_index_name + "\" is being created twice"
                );
            }
        }
    }
    ...
    // Default values
    const uint64_t dimension = std::stoull(ctx->INTEGER(0)->getText());
    if (dimension < 1) {
        throw QueryException("Dimension must be greater than 0");
    }

    const uint64_t max_edges = std::stoull(ctx->INTEGER(1)->getText());
    if (max_edges < 2) {
        throw QueryException("Max edges must be greater than 1");
    }

    const uint64_t num_candidates =  std::stoull(ctx->INTEGER(2)->getText());
    if (num_candidates < max_edges) {
        throw QueryException("Number of candidates must be greater than or equal to max edges");
    }

    const auto* transliterator = Transliterator::get_instance();
    const std::string metric_name = stringCtxToString(ctx->string(1));
    const std::string metric_name_lowercased = transliterator->lowercase(metric_name);

    // Validate if its a valid metric function
    HNSW::MetricType metric_type {};
    if (metric_name_lowercased == "euclideandistance") {
        metric_type = HNSW::MetricType::EUCLIDEAN_DISTANCE;
    } else if (metric_name_lowercased == "manhattandistance") {
        metric_type = HNSW::MetricType::MANHATTAN_DISTANCE;
    } else if (metric_name_lowercased == "cosinedistance") {
        metric_type = HNSW::MetricType::COSINE_DISTANCE;
    } else {
        throw QueryException("Unknown metric type: \"" + metric_name + "\"");
    }

    op_update->updates.emplace_back(std::make_unique<OpCreateHnswIndex>(
        std::move(hnsw_index_name),
        std::move(iri_str),
        dimension,
        max_edges,
        num_candidates,
