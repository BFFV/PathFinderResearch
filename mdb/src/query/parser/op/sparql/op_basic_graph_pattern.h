...
#include "query/parser/op/sparql/op_hnsw_search_index.h"
...
public:
    ...
    std::vector<OpHNSWSearchIndex> hnsw_searches;
    ...
    OpBasicGraphPattern(
        std::vector<OpTriple> triples_,
        std::vector<OpPath> paths_,
        std::vector<OpTextSearchIndex> text_searches_,
        std::vector<OpHNSWSearchIndex> hnsw_searches_
    ) :
        triples         (std::move(triples_)),
        paths           (std::move(paths_)),
        text_searches   (std::move(text_searches_)),
        hnsw_searches   (std::move(hnsw_searches_)) { }
    ...
    std::unique_ptr<Op> clone() const override
    {
        ...
        std::vector<OpHNSWSearchIndex> new_hnsw_searches;
        ...
        for (const auto& text_search : text_searches) {
            new_text_searches.emplace_back(text_search);
        }
        for (const auto& hnsw_search : hnsw_searches) {
            new_hnsw_searches.emplace_back(hnsw_search);
        }
        return std::make_unique<OpBasicGraphPattern>(
            std::move(new_triples),
            std::move(new_paths),
            std::move(new_text_searches),
            std::move(new_hnsw_searches)
        );
    }
    ...
    std::set<VarId> get_all_vars() const override
    {
        ...
        for (const auto& hnsw_search : hnsw_searches) {
            const auto vars = hnsw_search.get_all_vars();
            res.insert(vars.begin(), vars.end());
        }
        return res;
    }
    ...
    std::set<VarId> get_scope_vars() const override
    {
        ...
        for (const auto& hnsw_search : hnsw_searches) {
            const auto vars = hnsw_search.get_scope_vars();
            res.insert(vars.begin(), vars.end());
        }
        return res;
    }
    ...
    std::set<VarId> get_safe_vars() const override
    {
        ...
        for (const auto& hnsw_search : hnsw_searches) {
            const auto vars = hnsw_search.get_safe_vars();
            res.insert(vars.begin(), vars.end());
        }
        return res;
    }
    ...
    std::set<VarId> get_fixable_vars() const override
    {
        ...
        for (const auto& hnsw_search : hnsw_searches) {
            const auto vars = hnsw_search.get_fixable_vars();
            res.insert(vars.begin(), vars.end());
        }
        return res;
    }
    ...
    std::ostream& print_to_ostream(std::ostream& os, int indent = 0) const override
    {
        ...
        for (const auto& hnsw_search : hnsw_searches) {
            hnsw_search.print_to_ostream(os, indent + 2);
        }
        return os;
    }
};
