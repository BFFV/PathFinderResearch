...
void BindingIterConstructor::visit(OpQueryStatements& op_statements)
{
    // each child (OpLet, OpGraphPatternList, OpOrderByStatement, OpFilterStatement) should set the tmp iter
    for (auto& op : op_statements.ops) {
        op->accept_visitor(*this);
    }
}

void BindingIterConstructor::visit(OpFilterStatement& op_filter)
{
...
void BindingIterConstructor::visit(OpOrderByStatement& op_order_by)
{
...
void BindingIterConstructor::handle_order_by(
...
)
{
    ...
    if (!exprs_to_eval.empty()) {
        tmp = std::make_unique<ExprEvaluator>(std::move(tmp), std::move(exprs_to_eval));
    }
...
void BindingIterConstructor::visit(OpReturn& op_return)
{
    ...
    if (op_return.offset != Op::DEFAULT_OFFSET || op_return.limit != Op::DEFAULT_LIMIT) {
        tmp = std::make_unique<Slice>(std::move(tmp), op_return.offset, op_return.limit);
    }
}
...
void BindingIterConstructor::visit(OpLet& op_let)
{
    ...
    if (tmp != nullptr) {
        ...
        tmp = std::make_unique<SetVariableValues>(std::move(binding_exprs));
    }
}
...
void BindingIterConstructor::visit(OpGraphPatternList& op)
{
    ...
    auto previous_iter = std::move(tmp);
    ...
    if (previous_iter != nullptr) {
        tmp = std::make_unique<IndexNestedLoopJoin>(std::move(previous_iter), std::move(current_tmp));
    } else {
