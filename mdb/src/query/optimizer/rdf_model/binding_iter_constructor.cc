...
#include "query/executor/binding_iter/hnsw_index_scan_fixed_query.h"
...
std::vector<std::unique_ptr<BindingIter>>
    BindingIterConstructor::build_hnsw_searches(const std::vector<OpHNSWSearchIndex>& hnsw_searches)
{
    if (hnsw_searches.size() > 1) {
        throw NotSupportedException("Multiple HNSW Searches are not supported yet");
    }

    std::vector<std::unique_ptr<BindingIter>> hnsw_search_binding_iters;

    for (const auto& op_hnsw_search_index : hnsw_searches) {
        for (const auto& safe_var : op_hnsw_search_index.get_safe_vars()) {
            safe_assigned_vars.insert(safe_var);
        }

        auto& hnsw_index_manager = rdf_model.catalog.hnsw_index_manager;
        auto* hnsw_index_ptr = hnsw_index_manager.get_hnsw_index(op_hnsw_search_index.hnsw_index_name);
        assert(hnsw_index_ptr != nullptr && "the requested HNSW Index must exist");

        // Variable query
        if (op_hnsw_search_index.query_id.is_var()) {
            throw NotSupportedException("Unfixed HNSW queries are not supported yet");
        }

        // Fixed query
        hnsw_search_binding_iters.emplace_back(std::make_unique<HNSWIndexScanFixedQuery>(
            op_hnsw_search_index.object_var,
            op_hnsw_search_index.distance_var,
            op_hnsw_search_index.tensor_var,
            op_hnsw_search_index.query_id.get_OID(),
            op_hnsw_search_index.num_neighbors,
            op_hnsw_search_index.num_candidates,
            hnsw_index_ptr
        ));
    }

    return hnsw_search_binding_iters;
}

std::vector<std::unique_ptr<BindingIter>>
    BindingIterConstructor::build_text_searches(const std::vector<OpTextSearchIndex>& text_searches)
{
    // TODO: Variable query?
    if (text_searches.size() > 1) {
        throw NotSupportedException("Multiple Text Searches are not supported yet");
    }

    std::vector<std::unique_ptr<BindingIter>> text_search_binding_iters;

    for (const auto& op_text_search_index : text_searches) {
        for (const auto& safe_var : op_text_search_index.get_safe_vars()) {
            safe_assigned_vars.insert(safe_var);
        }
        ...
        if (op_text_search_index.text_search_index_name == "*") {
            // Text search over all text search indexes
            const auto& name2text_search_index = text_search_index_manager.get_name2text_search_index();
            assert(name2text_search_index.size() > 0 && "at least one TextSearchIndex must exists");

            std::vector<TextSearch::TextSearchIndex*> text_search_indexes;
            std::vector<ObjectId> predicate_ids;
            for (const auto& [name, text_search_index] : name2text_search_index) {
                const auto it = name2metadata.find(name);
                assert(it != name2metadata.end() && "the requested TextSearchIndex metadata must exist");
                const auto predicate_id = it->second.predicate_id;

                text_search_indexes.emplace_back(text_search_index.get());
                predicate_ids.emplace_back(predicate_id);
            }

            text_search_binding_iters.emplace_back(std::make_unique<TextSearchIndexMultiScan>(
                std::move(text_search_indexes),
                std::move(predicate_ids),
                op_text_search_index.query,
                op_text_search_index.search_type,
                op_text_search_index.object_var,
                op_text_search_index.match_var,
                op_text_search_index.predicate_var
            ));
            continue;
        }

        // Search over a single index
        auto* text_search_index_ptr = text_search_index_manager.get_text_search_index(
            op_text_search_index.text_search_index_name
        );
        assert(text_search_index_ptr != nullptr && "the requested TextSearchIndex should exist");

        auto it = name2metadata.find(op_text_search_index.text_search_index_name);
        assert(it != name2metadata.end() && "the requested TextSearchIndex metadata should exist");
        const auto predicate_id = it->second.predicate_id;
    ...
    return text_search_binding_iters;
}

void BindingIterConstructor::visit(OpBasicGraphPattern& op_basic_graph_pattern)
{
    // First, build index searches. They will be joined after building the BGP
    auto text_search_binding_iters = build_text_searches(op_basic_graph_pattern.text_searches);
    auto hnsw_search_binding_iters = build_hnsw_searches(op_basic_graph_pattern.hnsw_searches);
    ...
    // HNSW Searches
    if (!hnsw_search_binding_iters.empty()) {
        if (tmp != nullptr) {
            tmp = std::make_unique<IndexNestedLoopJoin>(
                std::move(hnsw_search_binding_iters[0]),
                std::move(tmp)
            );
        } else {
            tmp = std::move(hnsw_search_binding_iters[0]);
        }
    }
