...
void ExecutorConstructor::visit(OpShow& op_show)
{
    ...
    case SPARQL::ResponseType::CSV:
        if (op_show.type == OpShow::Type::TEXT_SEARCH_INDEX) {
            executor = std::make_unique<ShowExecutor<ResponseType::CSV, OpShow::Type::TEXT_SEARCH_INDEX>>();
            break;
        }
        [[fallthrough]];
    ...
    case SPARQL::ResponseType::TSV:
        if (op_show.type == OpShow::Type::TEXT_SEARCH_INDEX) {
            executor = std::make_unique<ShowExecutor<ResponseType::TSV, OpShow::Type::TEXT_SEARCH_INDEX>>();
            break;
        }
        [[fallthrough]];
