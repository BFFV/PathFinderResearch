...
ShowStreamingExecutor<type>::ShowStreamingExecutor()
{
    ...
    } else if constexpr (type == OpShow::Type::HNSW_SEARCH_INDEX) {
        projection_vars.emplace_back(get_query_ctx().get_or_create_var("name"));
        projection_vars.emplace_back(get_query_ctx().get_or_create_var("predicate"));
        projection_vars.emplace_back(get_query_ctx().get_or_create_var("dimension"));
        projection_vars.emplace_back(get_query_ctx().get_or_create_var("distance_metric"));
    } else {
...
uint64_t ShowStreamingExecutor<type>::execute(MDBServer::StreamingResponseWriter& response_writer)
{
    ...
    if constexpr (type == OpShow::Type::TEXT_SEARCH_INDEX) {
        ...
        const auto write_record = [&](const std::string& name, const TSIMetadata& metadata) {
        ...
        assert(projection_vars.size() == 4);
        using HSIMetadata = HNSW::HNSWIndexManager::HNSWIndexMetadata;

        const auto write_record = [&](const std::string& name,
                                      const HSIMetadata& metadata,
            ...
            response_writer.write_string(name, MDBServer::Protocol::DataType::STRING);
            response_writer.write_string(metadata.predicate, MDBServer::Protocol::DataType::IRI);
            response_writer.write_uint32(params.dimensions);
            response_writer.write_string(
        ...
        for (const auto& [name, metadata] : name2metadata) {
            const auto hnsw_index = hnsw_index_manager.get_hnsw_index(name);
            const auto params = hnsw_index->get_params();
...
template class SPARQL::ShowStreamingExecutor<SPARQL::OpShow::Type::HNSW_SEARCH_INDEX>;
