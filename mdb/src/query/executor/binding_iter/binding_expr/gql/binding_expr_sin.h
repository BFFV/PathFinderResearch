...
namespace GQL {
class BindingExprSin : public BindingExpr {
public:
    std::unique_ptr<BindingExpr> expr;

    BindingExprSin(std::unique_ptr<BindingExpr> expr) :
        expr(std::move(expr))
    { }
    ...
    ObjectId eval(const Binding& binding) override
    {
        ...
        case GQL_OID::GenericSubType::INTEGER: {
            ...
            return GQL::Conversions::pack_float(sin(expr));
        }
        case GQL_OID::GenericSubType::DECIMAL: {
            auto expr = GQL::Conversions::to_decimal(expr_oid);
            return GQL::Conversions::pack_decimal(expr.sin());
        }
        ...
        case GQL_OID::GenericSubType::DOUBLE: {
            ...
            return GQL::Conversions::pack_double(sin(expr));
        }
        default: {
            assert(false);
