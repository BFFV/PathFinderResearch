...
namespace GQL {
class BindingExprAcos : public BindingExpr {
public:
    std::unique_ptr<BindingExpr> expr;

    BindingExprAcos(std::unique_ptr<BindingExpr> expr) :
        expr(std::move(expr))
    { }
    ...
    ObjectId eval(const Binding& binding) override
    {
        ...
        case GQL_OID::GenericSubType::INTEGER: {
            ...
            return GQL::Conversions::pack_float(acos(expr));
        }
        case GQL_OID::GenericSubType::DECIMAL: {
            auto expr = GQL::Conversions::to_decimal(expr_oid);
            return GQL::Conversions::pack_decimal(expr.acos());
        }
        ...
        case GQL_OID::GenericSubType::DOUBLE: {
            ...
            return GQL::Conversions::pack_double(acos(expr));
        }
        default: {
            assert(false);
