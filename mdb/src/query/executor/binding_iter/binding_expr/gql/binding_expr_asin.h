...
namespace GQL {
class BindingExprAsin : public BindingExpr {
public:
    std::unique_ptr<BindingExpr> expr;

    BindingExprAsin(std::unique_ptr<BindingExpr> expr) :
        expr(std::move(expr))
    { }
    ...
    ObjectId eval(const Binding& binding) override
    {
        ...
        case GQL_OID::GenericSubType::INTEGER: {
            ...
            return GQL::Conversions::pack_float(asin(expr));
        }
        case GQL_OID::GenericSubType::DECIMAL: {
            auto expr = GQL::Conversions::to_decimal(expr_oid);
            return GQL::Conversions::pack_decimal(expr.asin());
        }
        ...
        case GQL_OID::GenericSubType::DOUBLE: {
            ...
            return GQL::Conversions::pack_double(asin(expr));
        }
        default: {
            assert(false);
