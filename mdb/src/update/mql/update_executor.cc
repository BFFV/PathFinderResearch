...
void UpdateExecutor::visit(OpInsert& op)
{
    ...
    for (const auto& [name, deletes] : text_search_index_name2deletes) {
        auto* text_search_index_ptr = quad_model.catalog.text_search_index_manager.get_text_search_index(name
        );
        ...
        for (const auto& [node, value] : deletes) {
            const auto current_deleted_tokens = text_search_index_ptr->remove_single(node, value);
    ...
    for (const auto& [name, inserts] : text_search_index_name2inserts) {
        ...
        for (const auto& [node, value] : inserts) {
            const auto current_inserted_tokens = text_search_index_ptr->index_single(node, value);
