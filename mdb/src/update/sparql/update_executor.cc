...
void UpdateExecutor::visit(SPARQL::OpInsertData& op_insert_data)
{
    ...
    boost::unordered_flat_map<std::string, std::vector<std::tuple<ObjectId, ObjectId>>>
    ...
    for (const auto& [name, inserts] : text_search_index_name2inserts) {
        auto* text_search_index_ptr = rdf_model.catalog.text_search_index_manager.get_text_search_index(name);
        assert(text_search_index_ptr != nullptr && "Text search index not found");
...
void UpdateExecutor::visit(SPARQL::OpDeleteData& op_delete_data)
{
    ...
    for (const auto& [name, deletes] : text_search_index_name2deletes) {
        ...
        for (const auto& [S, O] : deletes) {
            const auto current_removed_tokens = text_search_index_ptr->remove_single(S, O);
...
void UpdateExecutor::visit(SPARQL::OpCreateHnswIndex& op_create_hnsw_index)
{
    try {
        ...
        auto&& [inserted_elements] = hnsw_index_manager.create_hnsw_index(
            op_create_hnsw_index.hnsw_index_name,
        ...
        hnsw_update.hnsw_index_name = op_create_hnsw_index.hnsw_index_name;
...
{
    auto it = name2hnsw_index_update_data.find(data.hnsw_index_name);
    if (it == name2hnsw_index_update_data.end()) {
        name2hnsw_index_update_data.emplace(data.hnsw_index_name, std::move(data));
    } else {
        it->second.inserted_elements += data.inserted_elements;
    }
}
