...
SERVER_CMD = f"build/Release/bin/mdb server data/qm_wdbench --path-mode {PATH_MODE} --timeout {TIMEOUT} --threads 1 --load-strings 10GB --shared-buffer 64GB".split()
...
def parse_to_mql(query: str) -> None:
    ...
    mdb_semantic = PATH_SEMANTIC.replace("_", " ")

    # Parse subject
    if from_string[0] == "?":
        from_string = f"({from_string})=[{mdb_semantic} ?p "
    else:
        from_string = "(" + from_string.split("/")[-1].replace(">", "") + f")=[{mdb_semantic} ?p "
    ...
    with open(MDB_QUERY_FILE, "w", encoding="utf-8") as file:
...
def execute_queries(server_process: Popen[bytes]) -> Popen[bytes]:
    with open(QUERIES_FILE, encoding="utf-8") as queries_file:
        for line in queries_file:
            ...
            server_process = query_mdb(server_process, query, query_number)
    return server_process
...
def execute_mql_query(query_pattern: str, query_number: str, pid: int) -> None:
    ...
    with open(MDB_RESULTS_FILE, "w", encoding="utf-8") as results_file, open(
        MDB_QUERY_FILE, "r", encoding="utf-8"
    ) as query_file:
        query_execution = subprocess.Popen(
            ["./build/Release/bin/mdb-query-mql"], stdin=query_file, stdout=results_file, stderr=subprocess.DEVNULL
        )
...
def query_mdb(server_process: Popen[bytes], query_pattern: str, query_number: str) -> Popen[bytes]:
    start_time = time.time()
...
if __name__ == "__main__":
    ...
    RESUME_FILE = f"{MDB_ROOT}/out/{PATH_SEMANTIC}_{PATH_MODE}_limit{LIMIT}.csv"
    ERROR_FILE = f"{MDB_ROOT}/out/errors/{PATH_SEMANTIC}_{PATH_MODE}_limit{LIMIT}.log"
    SERVER_LOG_FILE = f"{MDB_ROOT}/out/log/{PATH_SEMANTIC}_{PATH_MODE}_limit{LIMIT}.log"

    MDB_QUERY_FILE = os.path.abspath(".wdbench_query.tmp")
    MDB_RESULTS_FILE = os.path.abspath(".wdbench_results.tmp")
