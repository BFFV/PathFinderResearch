...
The Text Search Index lets users create indexes based on a RDF predicate or MQL property to efficiently search and retrieve nodes from the database.
...
For querying in SPARQL we have the reserved IRI `http://millenniumdb.com/procedure/textSearch` (or for short `mdbproc:textSearch`) to instantiate a Text Search Index Scan. It receives as arguments:
...
WHERE {
    ?subject mdbproc:textSearch ("example" "Spider" "PREFIX" ?matchedText) .
}
```
...
- Expose the allow errors option for querying
- Support multiple Text Search Index Scans per Basic Graph Pattern
