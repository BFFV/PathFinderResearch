...
def start_server(db_name, path_mode):
    ...
    server = Popen(['build/Release/bin/mdb', 'server', database, '-t', f'{TIMEOUT}', '-p', f'{PORT}', '--path-mode', path_mode, '--no-browser',
                    '--versioned-buffer', '64GB', '--unversioned-buffer', '6GB', '--strings-static', '9GB'],
...
def execute_query(query_line, results_file, connection, server_proc):
    ...
    if exec_time <= TIMEOUT * 1000:
        with open(results_file, 'a') as out_file:
            out_file.write(f'{query_number},{query.replace(",", "..")},{n_results},OK,{exec_time},{enum_time},{memory_mb}\n')
        return False
