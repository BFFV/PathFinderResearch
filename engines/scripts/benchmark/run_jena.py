...
"""
Run: python run_jena.py [DB_NAME] [EXPERIMENT] [TIMEOUT_MAX]
...
ENGINE_DIR = os.path.join(ROOT_DIR, 'jena/apache-jena-fuseki')
...
def start_server(db_name):
    os.chdir(ENGINE_DIR)
    server = Popen(['java', '-Xmx64g', '-jar', 'fuseki-server.jar', f'--loc=../apache-jena/{db_name}', f'--timeout={TIMEOUT * 1000}', '/jena'],
...
def execute_query(query_line, results_file):
    ...
    # Execute new query
    sparql_wrapper = SPARQLWrapper(f'http://{IP}:{PORT}/jena/sparql')
...
if __name__ == '__main__':
    try:
        ...
        results_path = os.path.join(RESULTS_DIR, f'jena_{database_name}_{experiment}.csv')
