...
"""
Run: python run_virtuoso.py [DB_NAME] [EXPERIMENT] [TIMEOUT_MAX]
...
def start_server(db_name):
    os.chdir(ENGINE_DIR)
    server = Popen(['bin/virtuoso-t', '-c', f'{db_name}.ini', '+foreground'], stdin=DEVNULL, stdout=DEVNULL, stderr=DEVNULL)
...
if __name__ == '__main__':
    try:
        ...
        print(f'Engine: Virtuoso\nExperiment: {experiment}\nDatabase: {database_name}\n')
        query_path = os.path.join(QUERY_DIR, f'{database_name}_{experiment}.txt')
        results_path = os.path.join(RESULTS_DIR, f'virtuoso_{database_name}_{experiment}.csv')
