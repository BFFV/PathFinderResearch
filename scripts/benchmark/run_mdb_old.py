...
"""
Run: python run_mdb.py [DB_NAME] [MODE] [TIMEOUT_MAX] [SEARCH_STRATEGY]
    [DB_NAME] Name of the database
    [MODE] Semantic mode (
        0: ALL TRAILS, 1: ANY SHORTEST WALKS, 2: ALL SHORTEST WALKS, 3: ANY TRAILS, 4: ENDPOINTS, 5: ALL FIXED,
        6: ALL TRAILS FULL, 7: ANY SHORTEST WALKS FULL, 8: ALL SHORTEST WALKS FULL)
...
ENGINE_DIR = os.path.join(ROOT_DIR, 'mdb-old')
QUERY_DIR = os.path.join(ROOT_DIR, 'queries/mdb')
RESULTS_DIR = os.path.join(ROOT_DIR, 'results/mdb_old_test')
...
def start_server(db_name, path_mode):
    ...
    server = Popen(['build/Release/bin/mdb-server', database, '-t', f'{TIMEOUT}', '--path-mode', path_mode, '--shared-buffer', '64GB'],
...
def execute_query(query_line, results_file):
    ...
    with open('results.tmp', 'w') as out_file:
        start = time.time()
        run(['build/Release/bin/mdb-query-mql'], input=query, stdout=out_file, stderr=DEVNULL, text=True)
...
if __name__ == '__main__':
    try:
        ...
        modes = ['all_trails', 'any_shortest_walks', 'all_shortest_walks', 'any_trails', 'endpoints', 'all_fixed',
                 'full_all_trails', 'full_any_shortest_walks', 'full_all_shortest_walks']
